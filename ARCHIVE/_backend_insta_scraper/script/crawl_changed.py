...
class Crawler(object):
    ...
    def __init__(self, page_name=config.DEFAULT_PAGE_NAME, directory=config.DEFAULT_SAVE_DIR,
        ...
        self.browser = webdriver.Firefox(options=options)  #webdriver.Firefox()
    ...
    def run(self):
        ...
        if 1: #try:
            ...
            print('after crawl_general_page_info')

            self.basic_crawl()
            print('after basic_crawl')

            if authenticated:
                print('# authenticated, going to call "detailed_crawl"')
                self.detailed_crawl()

            print('going to call page.save')
            self.page.save()
            print('after save')
    ...
    def authentication(self):
        ...
        try:
            ...
            print('# authentication_url: '+str(authentication_url))
    ...
    def go_to_page(self, page):
        ...
        print('# going to page: '+str(page_url))
        self.browser.get(page_url)

    def crawl_general_page_info(self):
        print("Get general page info for", self.page_name)
        log.info("Get general page info for %s ", self.page_name)

        def get_instagram_field_number(expression):
            # Get instagram field from regular expression

            try:
                print('going to call group')
                #print('# page source: '+str(self.browser.page_source))
                num_info = regex.search(expression, self.browser.page_source).group()

                print('after calling group')
                field_number = int(regex.findall(r'\d+', num_info)[0])
            except AttributeError as e:
                print('# exception in get_instagram_field_number: '+str(e))
                return 0
        ...
        log.info("Obtained general page info for %s", self.page_name)
    ...
    def basic_crawl(self):
        ...
        log.info("Get Basic crawl info for %s", self.page_name)
        ...
        log.info("Obtained Basic crawl info for %s", self.page_name)
    ...
    def detailed_crawl(self):
        ...
        while (post_num < self.posts_number):
            print("inside the while in function detailed_crawl")
...
def main():
    ...
    try:
        ...
        print('# export_db: '+str(export_db))
        print('# photo_number: '+str(parsed_arg.photo_number))
        print('# page_name: '+str(parsed_arg.page_name))
        print('# more_details: '+str(parsed_arg.more_details))
        print('# directory: '+str(parsed_arg.export_db))
...
if __name__ == "__main__":
    ...
    if run_main: 
        ...
        crawler =   Crawler(page_name='thisisbillgates',
                            export_db=True, 
                            posts_number=30, 
                            more_details=True,
                            directory=True)
